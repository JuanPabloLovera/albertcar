...
export function useAssistantChat() {
  ...
  const [formData, setFormData] = useState(INITIAL_OIL_FORM);
  ...
  const startConversation = useCallback(async () => {
    ...
    try {
      ...
      setMessages([
        {
          role: "bot",
          content: "No fue posible iniciar el asistente.\nRevise la conexión e intente nuevamente.",
        },
      ]);
      setStep("start");
      setShowForm(false);
  ...
  const sendServiceSelection = useCallback(
    async (serviceLabel: string, _serviceValue: ServiceType) => {
      if (isLoading) return;
      ...
      try {
        ...
        pushBotMessage("No se pudo procesar la solicitud.\nIntente nuevamente.");
  ...
  const submitForm = useCallback(async () => {
    ...
    try {
      ...
      pushBotMessage("No se pudo enviar el formulario.\nIntente nuevamente.");
      setErrorMessage(toFriendlyError("network_error"));
    } finally {
      setIsLoading(false);
    }
